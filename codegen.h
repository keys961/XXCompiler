...
public:
    ...
    static void genIType(const std::string op, int dst, int src, int imm, const std::string label = "") {
        if (traceEmit) std::cout << "emit I op = " << op << " dst = " << dst << " src = " << src << std::endl;
        char ch[16] = {0,};
        sprintf(ch, "%d", imm);
        std::string c;
        std::string temp = " " + regTable[dst] + "," + regTable[src] + ",";
        enum option option1 = opMap[op];
        switch (option1) {
            case PLUS:
            case ASSIGN:
                c = "addi" + temp + ch;
...
//    }
    ...
    static void emitCodeM(const std::string op, int offset, int regAddr, int reg, int isFloat = 0) {
        if (traceEmit) std::cout << "emit M " << "op = " << op << " offset = " << offset << std::endl;
